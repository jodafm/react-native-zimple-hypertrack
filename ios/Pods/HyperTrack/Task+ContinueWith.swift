...
extension Task {
  /**
     ...
     */
  fileprivate func continueWithTask<S>(
    _ executor: Executor,
    ...
    let wrapperContinuation = {
      switch self.state {
      case .success where options.contains(.runOnSuccess),
        .error where options.contains(.runOnError),
        .cancelled where options.contains(.runOnCancelled):
        executor.execute {
          ...
          case .success(let nextTask):
            ...
            default: taskCompletionSource.setState(nextTask.state)
            }
          case .error(let error): taskCompletionSource.set(error: error)
          case .cancelled: taskCompletionSource.cancel()
          default:
      ...
      case .error(let error): taskCompletionSource.set(error: error)
      case .cancelled: taskCompletionSource.cancel()
      default: fatalError("Task was in an invalid state \(self.state)")
      }
    }
  ...
  /**
     ...
     */
  @discardableResult func continueWith<S>(
    _ executor: Executor = .default,
    continuation: @escaping ((Task) throws -> S)
  ) -> Task<S> {
    return continueWithTask(executor) { task in
      let state = TaskState.fromClosure({ try continuation(task) })
  ...
  /**
     ...
     */
  @discardableResult func continueWithTask<S>(
    _ executor: Executor = .default,
    continuation: @escaping ((Task) throws -> Task<S>)
  ) -> Task<S> {
...
extension Task {
  /**
     ...
     - returns: A task that will be completed when a task returned from a closure is completed.
     */
  @discardableResult func continueOnSuccessWith<S>(
    _ executor: Executor = .default,
    continuation: @escaping ((TResult) throws -> S)
  ) -> Task<S> {
    return continueOnSuccessWithTask(executor) { taskResult in
      let state = TaskState.fromClosure({ try continuation(taskResult) })
      return Task<S>(state: state)
    }
  }
  ...
  /**
     ...
     - returns: A task that will be completed when a task returned from a closure is completed.
     */
  @discardableResult func continueOnSuccessWithTask<S>(
    _ executor: Executor = .default,
    continuation: @escaping ((TResult) throws -> Task<S>)
  ) -> Task<S> {
...
extension Task {
  /**
     ...
     - returns: A task that will be completed when a task returned from a closure is completed.
     */
  @discardableResult func continueOnErrorWith<E: Error>(
    _ executor: Executor = .default,
    continuation: @escaping ((E) throws -> TResult)
  ) -> Task {
    return continueOnErrorWithTask(executor) { (error: E) in
      let state = TaskState.fromClosure({ try continuation(error) })
  ...
  /**
     ...
     - returns: A task that will be completed when a task returned from a closure is completed.
     */
  @discardableResult func continueOnErrorWith(
    _ executor: Executor = .default,
    continuation: @escaping ((Error) throws -> TResult)
  ) -> Task {
  ...
  /**
     ...
     - returns: A task that will be completed when a task returned from a closure is completed.
     */
  @discardableResult func continueOnErrorWithTask<E: Error>(
    _ executor: Executor = .default,
    continuation: @escaping ((E) throws -> Task)
  ) -> Task {
    return continueOnErrorWithTask(executor) { (error: Error) in
      if let error = error as? E { return try continuation(error) }
  ...
  /**
     ...
     - returns: A task that will be completed when a task returned from a closure is completed.
     */
  @discardableResult func continueOnErrorWithTask(
    _ executor: Executor = .default,
    continuation: @escaping ((Error) throws -> Task)
  ) -> Task {
