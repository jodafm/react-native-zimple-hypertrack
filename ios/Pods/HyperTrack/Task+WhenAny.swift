...
extension Task {
  ...
  /**
     ...
     */
  class func whenAny(_ tasks: [Task]) -> Task<Void> {
    ...
    for task in tasks {
      ...
      if taskCompletionSource.task.completed { break }
      task.continueWith { _ in taskCompletionSource.trySet(result: ()) }
    }
  ...
  /**
     ...
     - returns: A new task that will complete when any of the `tasks` are completed.
     */
  class func whenAny(_ tasks: Task...) -> Task<Void> { return whenAny(tasks) }
}
