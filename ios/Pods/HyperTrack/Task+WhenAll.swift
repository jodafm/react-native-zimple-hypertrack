...
extension Task {
  ...
  /**
     ...
     */
  class func whenAll(_ tasks: [Task]) -> Task<Void> {
    if tasks.isEmpty { return Task.emptyTask() }
    ...
    tasks.forEach {
      $0.continueWith { task -> Void in
        ...
        } else if task.faulted { OSAtomicIncrement32(&errorCount) }

        if OSAtomicDecrement32(&tasksCount) == 0 {
          if cancelledCount > 0 { tcs.cancel() } else if errorCount > 0 {
            #if swift(>=4.1)
              tcs.set(
                error: AggregateError(errors: tasks.compactMap({ $0.error }))
              )
            #else
              tcs.set(
                error: AggregateError(errors: tasks.flatMap({ $0.error }))
              )
            #endif
          } else { tcs.set(result: ()) }
        }
  ...
  /**
     ...
     - returns: A new task that will complete after all `tasks` are completed.
     */
  class func whenAll(_ tasks: Task...) -> Task<Void> { return whenAll(tasks) }
  ...
  /**
     ...
     */
  class func whenAllResult(_ tasks: [Task]) -> Task<[TResult]> {
    return whenAll(tasks).continueOnSuccessWithTask { task -> Task<[TResult]> in
      let results: [TResult] = tasks.map { task in
        guard let result = task.result else {
          ...
          // If a task errored, we should have returned a 'AggregateError'
          // and never gotten past 'continueOnSuccess'.
