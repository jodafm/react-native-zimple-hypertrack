...
protocol Localizable { func localizedValue() -> String }

/// An enumeration of all the different private data access levels.
/// Not every access level is applicable to every `PrivacyDataType`.
enum PrivateDataAccessLevel: String, Localizable {
  case unavailable = "UNAVAILABLE" // hardware level
  case undetermined = "UNDETERMINED"
  // permission not requested, or initial state
  ...
  // user has authorized when in use (location specific)
  case grantedAlways = "LOCATION_ALWAYS"
  // user has authorized always (location specific)

  func localizedValue() -> String {
    return NSLocalizedString(
      rawValue,
      comment: "Access level label for \(rawValue)"
    )
  }
}

/// A struct containing the results of prompting the user for access to a subset
...
struct PrivateDataRequestAccessResult: Localizable {
  ...
  init(
    _ accessLevel: PrivateDataAccessLevel,
    error: NSError? = nil,
  ...
  func localizedValue() -> String {
    ...
    if let errorMessageKey = errorMessageKey, let error = error {
      let localizedErrorFormatString = NSLocalizedString(
        errorMessageKey,
        comment: ""
      )
      let errorDescription = String(
        format: localizedErrorFormatString,
        error.code,
