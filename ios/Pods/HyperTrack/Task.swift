...
enum TaskState<TResult> {
  ...
  static func fromClosure(_ closure: () throws -> TResult) -> TaskState {
    do { return .success(try closure()) } catch is CancelledError {
      return .cancelled
...
struct TaskContinuationOptions: OptionSet {
  ...
  static let runAlways: TaskContinuationOptions = [
    .runOnSuccess, .runOnError, .runOnCancelled
  ]
}
...
///
final class Task<TResult> {
  typealias Continuation = () -> Void

  fileprivate let synchronizationQueue = DispatchQueue(
    label: "com.bolts.task",
  ...
  init(state: TaskState<TResult>) { _state = state }
  ...
  /**
     ...
     */
  init(_ result: TResult) { _state = .success(result) }
  ...
  /**
     ...
     */
  init(error: Error) { _state = .error(error) }
  ...
  class func cancelledTask() -> Self {
    // Swift prevents this method from being called `cancelled` due
    // to the `cancelled` instance var. This is most likely a bug.
  ...
  convenience init(
    ...
    closure: @escaping (() throws -> TResult)
  ) {
    self.init(state: .pending)
    executor.execute { self.trySet(state: TaskState.fromClosure(closure)) }
  }
  ...
  class func execute(
    _ executor: Executor = .default,
    closure: @escaping (() throws -> TResult)
  ) -> Task { return Task(executor, closure: closure) }
  ...
  /**
     ...
     - returns: A task that will continue with the task returned by the given closure.
     */
  class func executeWithTask(
    _ executor: Executor = .default,
    closure: @escaping (() throws -> Task)
  ) -> Task {
    return emptyTask().continueWithTask(executor) { _ in return try closure() }
  }
  ...
  func waitUntilCompleted() {
    if Thread.isMainThread {
      debugPrint(
        "Warning: A long-running operation is being executed on the main thread waiting on \(self)."
      )
    }
    ...
    synchronizationQueue.sync(
      flags: .barrier,
      execute: { self._completedCondition = nil }
    )
  }

  // MARK: State Change

  @discardableResult func trySet(state: TaskState<TResult>) -> Bool {
  ...
  func appendOrRunContinuation(_ continuation: @escaping Continuation) {
    ...
    synchronizationQueue.sync(
      ...
      execute: {
        switch self._state {
        case .pending: self._continuations.append(continuation)
        default: runContinuation = true
        }
      }
    )
    if runContinuation { continuation() }
  }

  var state: TaskState<TResult> {
    var value: TaskState<TResult>?
    synchronizationQueue.sync { value = self._state }
    return value!
  }
}
...
extension Task: CustomStringConvertible, CustomDebugStringConvertible {
  ...
  var description: String { return "Task: \(self.state)" }

  /// A textual representation of `self`, suitable for debugging.
  var debugDescription: String { return "Task: \(self.state)" }
}
