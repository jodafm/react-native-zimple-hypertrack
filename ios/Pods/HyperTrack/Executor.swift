...
enum Executor {
  ...
  func execute(_ closure: @escaping () -> Void) {
    ...
    case .mainThread:
      if Thread.isMainThread { closure() } else {
        DispatchQueue.main.async(execute: closure)
      }
    case .queue(let queue): queue.async(execute: closure)
    case .operationQueue(let operationQueue):
      operationQueue.addOperation(closure)
    case .closure(let executingClosure): executingClosure(closure)
...
extension Executor: CustomStringConvertible, CustomDebugStringConvertible {
  ...
  var description: String {
    switch self {
    case .default: return "Default Executor"
    case .immediate: return "Immediate Executor"
    case .mainThread: return "MainThread Executor"
    case .queue: return "Executor with dispatch_queue"
    case .operationQueue: return "Executor with NSOperationQueue"
    case .closure: return "Executor with custom closure"
    case .escapingClosure: return "Executor with custom escaping closure"
    }
  }
  ...
  var debugDescription: String {
    switch self {
    case .queue(let object): return "\(description): \(object)"
    case .operationQueue(let queue): return "\(description): \(queue)"
